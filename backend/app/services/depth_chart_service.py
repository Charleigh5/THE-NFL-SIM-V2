...
class DepthChartService:
    ...
    @staticmethod
    def get_starting_offense(
        players: list[Player], formation: str = "standard"
    ) -> dict[str, Player]:
        """
        ...
        def get_player(pos: str, rank: int) -> Player | None:
            if pos in chart and len(chart[pos]) > rank:
                return chart[pos][rank]
            # Fallback logic
            if pos == "OT" and "OG" in chart and len(chart["OG"]) > rank:
                return chart["OG"][rank]
            if pos == "OG" and "C" in chart and len(chart["C"]) > rank:
                return chart["C"][rank]  # Fallback C to OG
            if pos == "C" and "OG" in chart and len(chart["OG"]) > rank:
                return chart["OG"][rank]  # Fallback OG to C
            if pos == "TE" and "WR" in chart and len(chart["WR"]) > rank:
                return chart["WR"][rank]  # WR as TE
            if pos == "FB" and "RB" in chart and len(chart["RB"]) > rank:
                return chart["RB"][rank]  # RB as FB
            if pos == "WR" and "TE" in chart and len(chart["TE"]) > rank:
                return chart["TE"][rank]  # TE as WR
            return None
    ...
    def get_starting_defense(players: list[Player], formation: str = "4-3") -> dict[str, Player]:
        ...
        def get_player(pos: str, rank: int) -> Player | None:
            if pos in chart and len(chart[pos]) > rank:
                return chart[pos][rank]
            # Fallback
            if pos == "DE" and "LB" in chart and len(chart["LB"]) > rank:
                return chart["LB"][rank]  # LB as DE
            if pos == "DT" and "DE" in chart and len(chart["DE"]) > rank:
                return chart["DE"][rank]  # DE as DT
            if pos == "LB" and "S" in chart and len(chart["S"]) > rank:
                return chart["S"][rank]  # S as LB
            if pos == "CB" and "S" in chart and len(chart["S"]) > rank:
                return chart["S"][rank]  # S as CB
            if pos == "S" and "CB" in chart and len(chart["CB"]) > rank:
                return chart["CB"][rank]  # CB as S
            return None
    ...
    @staticmethod
    def get_backup(players: list[Player], position: str, depth: int = 1) -> Player | None:
        """
