...
class StorylineEventService:
    ...
    def check_for_storyline_triggers(
        self, db: Session, team_id: int, season_id: int, week: int
    ) -> list[Storyline]:
        """
    ...
    def _check_streak(
        self, team_id: int, season_id: int, week: int, events: list[RPGEvent]
    ) -> Storyline | None:
        ...
        if td_count >= 8:  # Lots of scoring
            if key not in self._active_storylines:
                storyline = Storyline(
                    storyline_type=StorylineType.HOT_STREAK, team_id=team_id, start_week=week
                )
                self._active_storylines[key] = storyline
            else:
                self._active_storylines[key].intensity = min(
                    5, self._active_storylines[key].intensity + 1
                )
            ...
            if key not in self._active_storylines:
                storyline = Storyline(
                    storyline_type=StorylineType.COLD_STREAK, team_id=team_id, start_week=week
                )
                self._active_storylines[key] = storyline
            else:
                self._active_storylines[key].intensity = min(
    ...
    def _check_breakout_player(
        self, db: Session, team_id: int, season_id: int, week: int, events: list[RPGEvent]
    ) -> Storyline | None:
    ...
    def generate_storyline_news(
        self, db: Session, storyline: Storyline, season_id: int, week: int
    ) -> NewsItem:
