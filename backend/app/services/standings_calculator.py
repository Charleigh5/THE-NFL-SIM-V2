...
class StandingsCalculator:
    ...
    def calculate_standings(self, season_id: int) -> list[TeamStanding]:
        ...
        games = self.db.execute(select(Game).where(Game.season_id == season_id)).scalars().all()
        ...
        for game in games:
            ...
            if home_id in team_stats and away_id in team_stats:
                ...
                if winner_id == home_id:
                    home_team["head_to_head"][away_id] = (
                        home_team["head_to_head"].get(away_id, 0) + 1
                    )
                elif winner_id == away_id:
                    away_team["head_to_head"][home_id] = (
                        away_team["head_to_head"].get(home_id, 0) + 1
                    )
        ...
        for stats in team_stats.values():
            ...
            stats["win_percentage"] = (
                (stats["wins"] + 0.5 * stats["ties"]) / total_games if total_games > 0 else 0.0
            )
            ...
            stats["division_win_pct"] = (
                (stats["division_wins"] + 0.5 * stats["division_ties"]) / div_games
                if div_games > 0
                else 0.0
            )
            ...
            # Conference Win %
            conf_games = (
                stats["conference_wins"] + stats["conference_losses"] + stats["conference_ties"]
            )
            stats["conference_win_pct"] = (
                (stats["conference_wins"] + 0.5 * stats["conference_ties"]) / conf_games
                if conf_games > 0
                else 0.0
            )
        ...
        for team_id, stats in team_stats.items():
            ...
            stats["strength_of_schedule"] = (
                round(opp_win_pct_sum / valid_opponents, 3) if valid_opponents > 0 else 0.0
            )
        ...
        standings = [TeamStanding(**data) for data in standings_with_ranks]
