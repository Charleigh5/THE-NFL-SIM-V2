...
class ScoutingEngine:
    ...
    def generate_report(
        self, true_attributes: dict[str, int], scout: ScoutProfile, visits: int = 1
    ) -> ScoutingReport:
        """
        Generate or update a scouting report.
        """
        report = ScoutingReport(player_id="PROSPECT_X", scout_id=scout.scout_id)
    ...
    def _calculate_tier(
        self, completion: float, attribute: str, scout: ScoutProfile
    ) -> KnowledgeTier:
    ...
    def _is_specialty_match(self, attribute: str, specialty: ScoutSpecialty) -> bool:
        ...
        if specialty == ScoutSpecialty.ATHLETICISM:
            return attribute in ["speed", "strength", "agility"]
        if specialty == ScoutSpecialty.QB_GURU:
            return attribute in ["throw_power", "throw_accuracy_mid"]
        if specialty == ScoutSpecialty.TRENCHES:
            return attribute in ["run_block", "pass_block", "power_moves"]
        if specialty == ScoutSpecialty.SKILL_POS:
            return attribute in ["catching", "route_running", "man_coverage"]
        return False
