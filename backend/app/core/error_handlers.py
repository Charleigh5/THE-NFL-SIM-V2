...
async def database_exception_handler(request: Request, exc: IntegrityError):
    ...
    return JSONResponse(status_code=status.HTTP_409_CONFLICT, content=error_response.model_dump())
...
async def database_operational_error_handler(request: Request, exc: OperationalError):
    ...
    logger.error(
        f"Database operational error: {exc}", extra={"request_id": get_request_id(request)}
    )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    logger.warning(
        f"Validation error: {exc.errors()}", extra={"request_id": get_request_id(request)}
    )
    ...
    error_response = ErrorResponse(
        ...
        error=ErrorDetail(code="VALIDATION_ERROR", message="The request data is invalid."),
        details=details,
        request_id=get_request_id(request),
    )
...
async def pydantic_validation_handler(request: Request, exc: ValidationError):
    ...
    logger.warning(
        f"Pydantic validation error: {exc.errors()}", extra={"request_id": get_request_id(request)}
    )
    ...
    error_response = ErrorResponse(
        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
        error=ErrorDetail(code="DATA_VALIDATION_ERROR", message="The data provided is invalid."),
        details=details,
        request_id=get_request_id(request),
    )
