...
def mock_db():
    ...
    db = MagicMock()
    # Setup default execute behavior to return a mock result
...
def test_process_contract_expirations_releases_expired_players(service, mock_db):
    ...
    # Setup mock execute
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = expiring_players
...
def test_process_contract_expirations_keeps_multi_year_contracts(service, mock_db):
    ...
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = multi_year_players
    mock_db.execute.return_value = mock_result
...
def test_generate_draft_order_creates_correct_number_of_picks(service, mock_db):
    ...
    with patch.object(service.standings_calculator, 'calculate_standings', return_value=standings):
        # Mock playoff matchup query (Super Bowl)
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = None # No SB matchup
        mock_db.execute.return_value = mock_result
...
@pytest.mark.asyncio
async def test_rookie_generator_creates_correct_count():
    ...
    result = await generator.generate_draft_class(season_id=1, count=100)
...
@pytest.mark.asyncio
async def test_rookie_generator_creates_players_with_correct_attributes():
    ...
    rookies = await generator.generate_draft_class(season_id=1, count=10)
...
def test_simulate_draft_fills_all_picks(service, mock_db):
    ...
    # 1. get_current_pick (select DraftPick)
    # 2. simulate_next_pick -> select Player (rookies)
    # 3. _get_team_needs -> select Player (team roster)
    ...
    def execute_side_effect(stmt):
        ...
        if "from draft_pick" in stmt_str:
            # Return next unpicked pick
            next_pick = next((p for p in picks if p.player_id is None), None)
    ...
    def execute_side_effect(stmt):
        ...
        if "from draft_pick" in stmt_str:
            ...
            if "is_rookie = :is_rookie_1" in stmt_str or "is_rookie = true" in stmt_str:
                ...
                # Team needs
                mock_result.scalars.return_value.all.return_value = team_players
    ...
    def execute_side_effect(stmt):
        ...
        if "from team" in stmt_str:
            ...
            if "count(*)" in stmt_str:
                 mock_result.scalar.return_value = 0 # Empty roster
            else:
...
def test_simulate_free_agency_signs_best_available_players(service, mock_db):
    ...
    def execute_side_effect(stmt):
        ...
        if "from team" in stmt_str:
            ...
            if "count(*)" in stmt_str:
                 mock_result.scalar.return_value = 51 # Need 2 more
            else:
                 mock_result.scalars.return_value.all.return_value = free_agents
...
@pytest.mark.asyncio
async def test_start_offseason_executes_all_steps(service, mock_db):
    ...
    def execute_side_effect(stmt):
        mock_result = MagicMock()
        # DraftPick query
        if "from draft_pick" in str(stmt).lower():
    ...
    with patch.object(service, 'process_contract_expirations') as mock_contracts, \
         ...
         patch.object(service.rookie_generator, 'generate_draft_class', new_callable=AsyncMock) as mock_rookies:
