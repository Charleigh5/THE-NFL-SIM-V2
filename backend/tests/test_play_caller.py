...
def test_aggressive_coach_fourth_down(base_context, rng):
    aggressive_caller = PlayCaller(rng=rng, aggression=0.9)
    base_context.down = 4
...
def test_conservative_coach_fourth_down(base_context, rng):
    conservative_caller = PlayCaller(rng=rng, aggression=0.1)
    base_context.down = 4
    base_context.distance = 1
    base_context.distance_to_goal = 40 # Opp 40
...
def test_advanced_coach_personality(base_context, rng):
    """Test new coach personality system with different play calling styles"""
    # West Coast offense - more short passes
    west_coast_caller = PlayCaller(rng=rng, aggression=0.6, run_pass_ratio=0.3)  # More pass-heavy
    base_context.down = 2
...
def test_situational_awareness_system(base_context, rng):
    ...
    desperate_caller = PlayCaller(rng=rng, aggression=0.8)
    pass_count = 0
    for _ in range(15):
...
def test_opponent_weakness_exploitation(base_context, rng):
    ...
    exploit_caller = PlayCaller(rng=rng, aggression=0.7, run_pass_ratio=0.2)  # Very pass-heavy
...
def test_adaptive_strategy_system(base_context, rng):
    ...
    clock_management_caller = PlayCaller(rng=rng, aggression=0.3)  # Conservative for clock management
    ...
    assert run_count >= 7  # Should favor running when protecting lead (accounting for variance)
