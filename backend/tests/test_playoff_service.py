...
def test_get_bracket(service, mock_db):
    ...
    def execute_side_effect(stmt):
        ...
        if "playoff_matchup" in query_str:
            mock_result.scalars.return_value.all.return_value = expected_matchups
            return mock_result
        return mock_result
...
def test_advance_round_incomplete_round(service, mock_db):
    ...
    def execute_side_effect(stmt):
        ...
        # Check for PlayoffMatchup first (more specific)
        if "playoff_matchup" in query_str:
            mock_result.scalars.return_value.all.return_value = wc_matchups
            return mock_result

        # Check for Season (and ensuring it's not matching season_id parameter in other queries)
        if "season" in query_str:
...
def test_advance_round_divisional_to_conference(service, mock_db):
    ...
    def execute_side_effect(stmt):
        ...
        if "playoff_matchup" in query_str:
             mock_result.scalars.return_value.all.return_value = div_matchups
             return mock_result
...
def test_advance_round_conference_to_super_bowl(service, mock_db):
    ...
    def execute_side_effect(stmt):
        ...
        if "playoff_matchup" in query_str:
             mock_result.scalars.return_value.all.return_value = conf_matchups
             return mock_result
