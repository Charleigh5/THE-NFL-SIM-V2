...
2. **State Management:** Tracks dynamic state that changes _during_ the game (Fatigue, Injury, Morale, Substitutions).
...
- **New:** `GenesisKernel` instance _owned_ by `MatchContext`.
...
- **Current:** Placeholder.
- **New:** `CortexKernel` instance _owned_ by `MatchContext`.
