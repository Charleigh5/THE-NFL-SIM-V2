...
import type { PlayResult, ClipCue, OverlayCue } from "../types/broadcast";
...
export const LiveGameVisualizer = ({
  ...
  const directorRef = useRef(getCutsceneDirector());
  const [activeClip, setActiveClip] = useState<ClipCue | null>(null);
  const [activeOverlays, setActiveOverlays] = useState<OverlayCue[]>([]);

  // Mock play result for testing (in real app, this comes from simulation)
  const mockPlayResult: PlayResult = {
    ...
    passerId: 1001,
    receiverId: 1005,
    tacklerIds: [2003, 2007],
    ballCarrierId: 1005,
    startTime: Date.now() / 1000,
    endTime: Date.now() / 1000 + 5.5,
  ...
  // Broadcast system integration (only when enableBroadcast=true)
  useEffect(() => {
    if (!enableBroadcast) return;

    // Initialize broadcast state machine
    broadcastStore.dispatch({ type: "PLAY_CALLED", playResult: mockPlayResult });

    // Generate clip sequence for current phase
    const director = directorRef.current;
    const currentPhase = broadcastStore.getState().phase;
    const clips = director.generateClipSequence(mockPlayResult, currentPhase);
    
    if (clips.length > 0) {
      setActiveClip(clips[0]);
      setActiveOverlays(clips[0].overlays || []);
    }

    // Auto-advance through phases for demo
    const phaseTimeout = setTimeout(() => {
      broadcastStore.dispatch({ type: "SNAP" });
      
      setTimeout(() => {
        broadcastStore.dispatch({ type: "WHISTLE", playResult: mockPlayResult });
        
        // Determine next phase
        const nextPhase = director.determineNextPhase(
          BroadcastPhase.POST_PLAY,
          mockPlayResult
        );
        
        if (nextPhase === BroadcastPhase.REPLAY) {
          broadcastStore.dispatch({ type: "REPLAY_REQUESTED" });
        } else {
          broadcastStore.dispatch({ type: "REPLAY_COMPLETE" });
        }
      }, 5500); // Simulate play duration
    }, 4500); // Pre-play duration

    return () => clearTimeout(phaseTimeout);
  }, [enableBroadcast, broadcastStore]);

  // Camera interpolation based on active clip
  useEffect(() => {
    if (!enableBroadcast || !activeClip || activeClip.cameras.length === 0) return;

    let frameId: number;
    let startTime: number | null = null;
    const cameraIndex = broadcastStore.getState().currentCameraIndex;
    const shots = activeClip.cameras;

    const animateCamera = (timestamp: number) => {
      if (!startTime) startTime = timestamp;
      const elapsed = (timestamp - startTime) / 1000; // seconds

      // Simple camera interpolation logic (would be more sophisticated in production)
      if (elapsed < activeClip.duration) {
  ...
  }, [enableBroadcast, activeClip, broadcastStore]);
