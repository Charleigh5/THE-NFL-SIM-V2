...
export const EnhancedFieldVisualizer = ({
  gameId,
  showPlayers = true,
  detailLevel = "medium",
  homeColor = "#002244",
  ...
  showYardLines = true,
  showNumbers = true,
}: EnhancedFieldVisualizerProps) => {
  const fieldRef = useRef<THREE.Group>(null);
  
  // Animation for stadium lights flicker
  useFrame((state) => {
  ...
  return (
    <group ref={fieldRef}>
      ...
      {Array.from({ length: 21 }).map((_, i) => {
        const x = (i - 10) * 6;
        const yardNumber = i === 0 ? 0 : i === 10 ? 50 : i === 20 ? 100 : 50 - Math.abs(x / 6) * 5;
      ...
      <group position={[-70, 0, 0]}>
        ...
        <mesh position={[0, 10, 0]} castShadow>
          <cylinderGeometry args={[0.15, 0.15, 18.5, 8]} rotation={[0, 0, Math.PI / 2]} />
          <meshStandardMaterial color="yellow" roughness={0.5} metalness={0.6} />
        </mesh>
      ...
      {/* Stadium seating (simplified background) */}
      {detailLevel === "high" && (
        <>
      ...
      {detailLevel !== "low" && (
        <>
          <pointLight position={[0, 30, 0]} intensity={1.2} color="#fff5e6" castShadow distance={150} />
