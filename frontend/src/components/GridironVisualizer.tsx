...
export interface GridironVisualizerProps {
  ...
  width?: number;
  height?: number;
  onCellHover?: (cellInfo: { yardX: number; yardY: number; friction: number; wear: number } | null) => void;
}
...
function generateDefaultTurfGrid(): TurfGridData {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      const wear = isCenter && isMidfield ? 0.35 + Math.random() * 0.45 : Math.random() * 0.20;
      const friction = Math.max(0.70, 1.0 - wear * 0.30);
      row.push({
        gridX: c,
        gridY: r,
...
export const GridironVisualizer: React.FC<GridironVisualizerProps> = ({
  ...
  const [hoveredMetric, setHoveredMetric] = useState<{ yardX: number; yardY: number; friction: number; wear: number } | null>(null);
  ...
  }, [width, height, scaleX, scaleY, toCanvasX, toCanvasY, showHeatmap, turfGrid, showVisionCones, players, selectedPlayer, showCognitiveBadges, ballPosition]);
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3 border-t border-slate-800 text-xs font-mono">
        ...
        <div className="bg-slate-950/70 p-2.5 rounded border border-slate-800">
          ...
          {hoveredMetric ? (
            <div className="flex justify-between">
              ...
              <span>
                Friction: <strong className="text-amber-400">μ={(hoveredMetric.friction).toFixed(2)}</strong>
              </span>
              <span>
                Wear: <strong className="text-red-400">{Math.round(hoveredMetric.wear * 100)}%</strong>
              </span>
            </div>
          ) : (
        ...
        <div className="bg-slate-950/70 p-2.5 rounded border border-slate-800 md:col-span-2">
          <span className="text-slate-500 uppercase block mb-1">Selected Athlete S2 Cognition Profile</span>
          {selectedPlayer ? (
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="text-slate-400">
                S2 Latency: <strong className="text-sky-300">{selectedPlayer.s2LatencyMs} ms</strong>
              </span>
              <span className="text-slate-400">
                Vision Cone: <strong className="text-slate-200">{selectedPlayer.visionConeAngleDeg}°</strong>
              </span>
            </div>
          ) : (
            <span className="text-slate-600 italic">Click on any player sprite to inspect cognitive telemetry</span>
          )}
        </div>
      </div>
    </div>
