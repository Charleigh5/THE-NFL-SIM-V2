...
test.describe("Front Office Journey", () => {
  test.beforeEach(async ({ page }) => {
    // Satisfy router loader guards (frontOfficeLoader checks selectedTeamId)
  ...
  test("should load front office with team roster", async ({ page }) => {
    ...
    // Verify team name
    await expect(page.locator('[data-testid="front-office-header"]')).toContainText(
      "Arizona Cardinals"
    );

    // Verify roster count
    // mockPlayers has 2 players
    await expect(page.locator('[data-testid="roster-section"]')).toContainText("Active Roster (2)");
  ...
  test("should open player detail modal", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="player-speed"]')).toBeVisible();
    await expect(page.locator('[data-testid="player-strength"]')).toBeVisible();
