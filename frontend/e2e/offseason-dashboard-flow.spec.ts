...
test.describe("Offseason Dashboard Flow", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/settings", async (route) => {
      await route.fulfill({
        json: { user_team_id: mockTeam.id, difficulty_level: "Pro", game_speed: "medium" },
      });
    });
  ...
  test("should load offseason dashboard and progress through phases", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="player-progression-card"]')).toContainText(
      "Kyler Murray"
    );
    ...
    await expect(page.locator('[data-testid="draft-pick-item-1"]')).toContainText(
      "Rd 1 Pick 1 QB Top QB"
    );
