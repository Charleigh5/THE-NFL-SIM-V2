...
test.describe("Team Selection Flow", () => {
  ...
  test("should display all teams and allow selection", async ({ page }) => {
    ...
    await expect(
      page.locator("p", { hasText: "Pick the badge you’ll carry into the storm lights." })
    ).toBeVisible();
    ...
    const teamCard = page.locator(
      `.team-card:has-text("${teamToSelect.city} ${teamToSelect.name}")`
    );
