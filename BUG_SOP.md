...
1. Read page-specific BUG_SOLUTIONS.md (e.g., docs/troubleshoot/frontend/BUG_SOLUTIONS.md for Frontend)
2. Search for similar bugs - check all relevant entries for the feature (simulation, roster, api, etc.)
...
- **[Frontend (React)](./docs/troubleshoot/frontend/BUG_SOLUTIONS.md)** - Frontend UI specific bugs and solutions
- **[Backend (FastAPI)](./docs/troubleshoot/backend/BUG_SOLUTIONS.md)** - Backend API and Kernel bugs
- **[Database/Models](./docs/troubleshoot/database/BUG_SOLUTIONS.md)** - Database schema and persistence bugs
...
| Async DB State | Commit called too late or on detached instance | Use SessionLocal context managers | Backend Services |

> **Note**: Most bugs are component-specific. See page-specific solutions above for detailed bug fixes.
...
5. **Use AsyncMock for Testing** - When testing backend async functions, always use `AsyncMock` to avoid coroutine mismatch errors.
...
5. **Using Synchronous DB calls in Async Routes** - Blocks the event loop.
...
1. **For page-specific bugs**: Navigate to the relevant page-specific solution (e.g., [Frontend](./docs/troubleshoot/frontend/BUG_SOLUTIONS.md))
